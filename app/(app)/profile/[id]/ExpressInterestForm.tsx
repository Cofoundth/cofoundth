...
export function ExpressInterestForm({
  ...
  if (relationship === "incoming") {
    return (
      <div className="bg-cream p-6 rounded-3xl">
        ...
        {state?.error && (
          <div className="px-4 py-3 mb-3 border border-danger-line bg-danger-surface text-sm text-danger-ink rounded-xl">
            {state.error}
          </div>
        )}
  ...
  return (
    <form
      action={formAction}
      className="bg-white p-6 space-y-4 rounded-3xl shadow-xs"
    >
      <input type="hidden" name="toId" value={toId} />
      <div>
        <label
