...
export function ExpressInterestForm({
    ...
    return (
      <div className="bg-cream border border-line p-6 text-center rounded-xl">
        ...
        <Link
          ...
          className="w-full px-6 py-3 bg-navy hover:bg-navy-dark text-white text-sm tracking-wide transition-colors inline-flex items-center justify-center gap-2 rounded-full"
        >
  ...
  if (relationship === "incoming") {
    return (
      <div className="bg-cream border border-line p-6 rounded-xl">
        ...
        {state?.error && (
          <div className="px-4 py-3 mb-3 border border-red-300 bg-red-50 text-sm text-red-800 rounded-xl">
            {state.error}
          </div>
        )}
  ...
  return (
    <form
      action={formAction}
      className="bg-white border border-line p-6 space-y-4 rounded-xl"
    >
      <input type="hidden" name="toId" value={toId} />
      <div>
        <label
        ...
        <textarea
          ...
          className="w-full px-4 py-3 border border-line bg-white text-ink text-sm focus:outline-none focus:border-navy resize-none rounded-xl"
        />
      </div>
      ...
      <div className="flex gap-3">
        <button
          ...
          disabled={isPending}
          className="flex-1 px-6 py-3 border border-line bg-white hover:border-navy text-ink text-sm tracking-wide transition-colors rounded-xl"
        >
          {tr("Cancel")}
        </button>
        <button
